...
### Редактор актов в Web UI

Во вкладке **Редактор актов** доступно:
- список актов workspace с действиями **Редактировать**, **Удалить**, **Создать акт**;
- редактирование метаданных акта: шаблон, имя файла, `output_name`, номер (prefix + value);
- редактирование динамических полей акта (добавление/удаление);
- типы полей: `text`, `multiline`, `date`, `number`;
- загрузка переменных шаблона и подсветка заполненности;
- кнопка **Добавить недостающие поля** из шаблона;
- показ наследуемых полей из `project.md` и переопределение на уровне акта;
- генерация DOCX прямо из редактора (**Сохранить + build-one + Открыть**).

Защита от потери данных:
- предупреждение при переходе на другой акт/экран с несохраненными изменениями;
- предупреждение браузера при обновлении страницы или закрытии вкладки.

Ограничения текущей версии редактора:
- тип `reference` временно скрыт в UI до отдельной доработки;
